      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-18">
          <Link to="/" className="flex items-center">
            <img 
              src={logo1}
              alt="VTU Material" 
              className="h-18 w-auto transform transition-transform duration-300 hover:scale-110 hover:rotate-4"
            />
          </Link>
          ...
          <div className="hidden md:flex md:items-center md:gap-6">
            ...
            {user ? (
              ...
              <Link to="/login" className="ml-4 bg-white text-blue-600 px-4 py-2 rounded-md hover:bg-gray-100 font-medium">
                Login
              </Link>
            )}
          </div>
        ...
        <div className={`${isOpen ? 'block' : 'hidden'} md:hidden pb-4 `}>
          <div className="flex flex-col  gap-2">
            <NavLink to="/" className={({ isActive }) => 
              `${isActive ? "text-blue-600 bg-gray-50" : "text-black"} px-3 py-2 rounded-md hover:bg-blue-600 hover:text-white`
            }>HOME</NavLink>
            <NavLink to="/about" className={({ isActive }) => 
              `${isActive ? "text-blue-600 bg-gray-50" : "text-black"} px-3 py-2 rounded-md hover:bg-blue-600 hover:text-white`
            }>ABOUT</NavLink>
            ...
            {user ? (
              <>
                {user.role === "admin" && (
                  <Link to="/admin" className="text-white hover:bg-blue-600 px-3 py-2 rounded-md">
                    ADMIN
                  </Link>
                )}
                <button
              ...
              <Link to="/login" className="text-blue-600 bg-white px-3 py-2 rounded-md hover:bg-gray-100 mt-2">
                Login
              </Link>
            )}
          </div>
